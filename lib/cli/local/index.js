...
module.exports.builder = function(yargs) {
  ...
  return yargs
    ...
    .option('wda-path', {
      describe: 'The WebDriverAgent Project path.'
    , type: 'string'
    , default: path.resolve('./')+'/WebDriverAgent/'
    })
    .option('wda-port', {
      describe: 'The WDA Server port on device.'
    , type: 'string'
    , default: '8100'
    })
    .option('wda-mjpeg-port', {
      describe: 'The WDA Mjpeg Server port on device.'
    , type: 'string'
    , default: '9100'
    })
    .option('allow-remote', {
...
module.exports.handler = function(argv) {
  ...
  function run() {
    ...
    , procutil.fork(path.resolve(__dirname, '..'), [
      'ios-provider'
    , '--name', argv.provider
    , '--min-port', 6100//argv.providerMinPort
    , '--max-port', 6900//argv.providerMaxPort
    , '--connect-sub', argv.bindDevPub
    , '--connect-push', argv.bindDevPull
    , '--group-timeout', argv.groupTimeout
    , '--public-ip', argv.publicIp
    , '--storage-url'
    , util.format('http://localhost:%d/', argv.port)
    , '--wda-path', argv.wdaPath
    , '--wda-port',argv.wdaPort
    , '--wda-mjpeg-port',argv.wdaMjpegPort
    , '--vnc-initial-size', argv.vncInitialSize.join('x')
    , '--mute-master', argv.muteMaster
    ]
    .concat(argv.allowRemote ? ['--allow-remote'] : [])
